...
public class SqlSiteMapProvider : StaticSiteMapProvider
{
    ...
    private string _connect, sqlFilter, urlPrefix, sqlFlatNav, dependency;   // Database connection string and sqlFilter
    ...
    private int _indexID, _indexTitle, _indexUrl, _indexDesc, _indexRoles, _indexParent, _indexAccessKey;
	...
	public override void Initialize (string name, NameValueCollection config)
	{
        ...
        dependency = config["sqlCacheDependency"];
		...
		urlPrefix = "~page.aspx?key=";

		if (config["urlprefix"] != null)
		{
			urlPrefix = config["urlprefix"];
			config.Remove("urlprefix");
		}


        sqlFlatNav = "";

        if (config["flatnav"] != null)
        {
            sqlFlatNav = config["flatnav"];
            config.Remove("flatnav");
        }
	...
	public override SiteMapNode BuildSiteMap()
	{
        lock (_lock)
        {
            ...
            if (_root != null)
            {
                if (dependency == null) // no cache so clear any session stuff
                {
                    Clear();
                    _nodes.Clear();
                    _root = null;
            ...
            try
            {
				...
				command.Parameters.AddWithValue("filter", sqlFilter);
				command.Parameters.AddWithValue("urlprefix", urlPrefix);
				//command.Parameters.AddWithValue("flatnav", sqlFlatNav);
                ...
                if (reader.Read())
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        if (GetParentNodeFromDataReader(reader) != null){
    ...
    private SiteMapNode GetParentNodeFromDataReader(DbDataReader reader)
    {
        ...
        if (!_nodes.ContainsKey(pid))
            return null;
            //throw new ProviderException(_errmsg4 + " " + pid + "node id " + reader.GetString(_indexTitle));
