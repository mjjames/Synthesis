...
namespace mjjames.AdminSystem.Repositories
{
	public class KeyValueRepository
	{
		...
		public void UpdateKeyValues(IEnumerable<KeyValueData> data)
		{
			...
			var updateData = (from kv in _admin.keyvalues
			                 where data.Select(k => k.LinkKey).Contains(kv.link_fkey) 
							 	&& data.Select(k => k.LinkTypeID).Contains(kv.lookup1.lookup_id)
			                       && data.Select(k => k.LookupID).Contains(kv.lookup.lookup_id)
			                 select kv).ToDictionary(kv => String.Format("{0}-{1}-{2}", kv.link_fkey, kv.lookup.lookup_id, kv.lookup1.lookup_id),
												 kv => kv);
			...
			{
				var key = String.Format("{0}-{1}-{2}", keyvalue.LinkKey, keyvalue.LookupID, keyvalue.LinkTypeID);
		...
		public void AddKeyValues(IEnumerable<KeyValueData> data)
		{
			...
			var lookupIDs = data.Select(k => k.LinkTypeID).Union(data.Select(k => k.LookupID)).ToList();
			var lookupDict = (from l in _admin.lookups
			                 where lookupIDs.Contains(l.lookup_id)
			                 select new
			                        	{
			                        		l.lookup_id,
			                        		l.lookup_key
			                        	}).ToDictionary(k => k.lookup_id, k => k.lookup_key);
			...
			var dataToInsert = data.Select(k => new keyvalue
			                                    	{
			                                    		link_fkey = k.LinkKey,
			                                    		value = k.Value,
			                                    		key_lookup = lookupDict[k.LinkTypeID],
			                                    		link_lookup = lookupDict[k.LookupID]
			                                    	});
